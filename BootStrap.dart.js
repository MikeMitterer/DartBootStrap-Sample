...
$dynamic("set$paddingLeft").CSSStyleDeclaration = function(value) {
...
BootStrap.prototype.run = function() {
  var $this = this; // closure support
  get$$document().get$window().setInterval((function () {
    $this._write();
  })
  , (1000));
}
BootStrap.prototype._write = function() {
  var color = this._colors.$index($mod$(this._counter, this._colors.get$length()));
  get$$document().query("#headline").set$innerHTML($add$(this._text, " -> ") + this._counter);
  get$$document().query("#headline").style.set$color(color);
  var $$list = get$$document().queryAll("h2");
  for (var $$i = $$list.iterator(); $$i.hasNext(); ) {
    var element = $$i.next();
    element.get$style().set$color(color);
  }
  var $$list = get$$document().queryAll("h1");
  for (var $$i = $$list.iterator(); $$i.hasNext(); ) {
    var element = $$i.next();
    element.get$style().set$paddingLeft($add$((($mod$(this._counter, (10))) * (3)).toString(), "px"));
  }
  this._counter++;
}
// ********** Code for top level **************
